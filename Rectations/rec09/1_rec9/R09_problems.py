...
class SwimWorkout(Workout):
    ...
    def get_calories(self):
        ...
        if (self.calories == None):
            ...
            return self.calories
    def count_laps(self):
        return self.numlaps + 1

    def get_average_laptime(self):
        if self.numlaps == 0:
            return None
        else:
            top = (self.end - self.start).total_seconds()
            bottom = self.numlaps
            return f"{top / bottom} seconds"
    ...
    def __str__(self):
        ...
        retstr += f"| {self.icon}{' '*(width-3)}|\n"  #assume width of icon is 2 chars - len('🏃🏽‍♀️');  doesn't do what you'd expect
        retstr += f"| {self.type}{' '*(width-len(self.type)-1)}|\n"
        retstr += f"| {self.numlaps} laps {' '*(width-8)}|\n"
        retstr += f"|{' ' *width}|\n"
        duration_str = str(self.get_duration())
        retstr += f"| {duration_str}{' '*(width-len(duration_str)-1)}|\n"
        cal_str = f"{round(self.get_calories(),1)}"
...
start = '9/30/2021 2:20 PM'
end = '9/30/2021 2:50 PM'
# 2) Swim 40 laps
laps = 40

a = SwimWorkout(start,end,numlaps=laps)

for i in range(40):
    a.count_laps()
   
# 3) Print average laptime
print(a.get_average_laptime())
